...
def analyze_data_distribution(dataset, logger, writer):
    """Analyze the distribution of safety scores in the dataset."""
    safety_scores = dataset.safety_scores.squeeze().numpy()
    
    # Calculate statistics
    unique_scores, counts = np.unique(safety_scores, return_counts=True)
    total_samples = len(safety_scores)
    
    logger.info("=== Data Distribution Analysis ===")
    logger.info(f"Total samples: {total_samples}")
    logger.info(f"Unique safety scores: {unique_scores}")
    logger.info(f"Counts: {counts}")
    percentages = 100 * counts / total_samples
    logger.info(f"Percentages: {percentages[0]:.1f}%, {percentages[1]:.1f}%")
    
    # Check for class imbalance
    if len(unique_scores) == 2:
        class_ratio = counts[1] / counts[0] if counts[0] > 0 else float('inf')
        logger.info(f"Class ratio (safe/unsafe): {class_ratio:.3f}")
        
        if class_ratio > 10 or class_ratio < 0.1:
            logger.warning("⚠️  Severe class imbalance detected!")
        elif class_ratio > 3 or class_ratio < 0.33:
            logger.warning("⚠️  Moderate class imbalance detected!")
    
    # Check for constant predictions
    if len(unique_scores) == 1:
        logger.error("❌ CRITICAL: All safety scores are the same! This will cause training issues.")
        return False
    
    # Log to tensorboard
    for score, count in zip(unique_scores, counts):
        writer.add_scalar(f'Data/Safety_Score_{score}', count, 0)
...
def main():
    ...
    parser.add_argument('--train-data', type=str, 
                      default='/home/othman/fetchReachExp/example_safety_data_0.2_full_episodes/safety_data_final.pkl',
                      help='Path to training data')
    parser.add_argument('--val-data', type=str,
                      default='/home/othman/fetchReachExp/example_safety_data_0.2_full_episodes_eval/safety_data_final.pkl',
                      help='Path to validation data')
    parser.add_argument('--output-dir', type=str, default='safety_models',
    ...
    if not analyze_data_distribution(train_dataset, logger, writer):
        logger.error("Training data has issues. Stopping training.")
        return
    
    logger.info("Analyzing validation data distribution...")
    if not analyze_data_distribution(val_dataset, logger, writer):
        logger.error("Validation data has issues. Stopping training.")
        return
    ...
    for epoch in range(args.epochs):
        ...
        if epoch > 10:
            ...
            if len(set([round(l, 4) for l in recent_val_losses])) == 1:
                logger.warning("⚠️  Validation loss has been constant for 10 epochs. Possible issues:")
                logger.warning("   - Model might be overfitting")
                logger.warning("   - Learning rate might be too low")
                logger.warning("   - Data might have issues")
                logger.warning("   - Model capacity might be insufficient")
