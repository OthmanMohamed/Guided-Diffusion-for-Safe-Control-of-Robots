...
def build_env(task, render_mode="rgb_array"):
    """Build the environment with the specified task."""
...
def collect_safety_data(args):
    ...
    env = build_env(args.env, render_mode=render_mode)
    ...
    with tqdm(total=args.num_episodes, desc="Collecting episodes") as pbar:
        while total_episodes < args.num_episodes:
            # Reset environment
            obs, _ = env.reset(seed=args.random_seed + total_episodes if args.random_seed is not None else None)
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='safety_data',
    ...
    # Collect data
    collected_data = collect_safety_data(args)
